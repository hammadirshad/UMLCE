...
public class ActivityDiagram extends UMLCE implements Serializable {
	...
	public Result evaluateModel(ModelFile solutionModel,
		...
		String response = port.evaluateActivityDiagram(
				IOUtils.toByteArray(solutionModel.getModel()),
	...
	public Result identifyMistakes(ModelFile model) throws Exception {
		...
		String response = port.activityDiagramMistakes(IOUtils
	...
	public Result checkPlagiarism(ModelFile model) throws Exception {
		...
		String response = port.checkActiityDiagramPlagiarism(IOUtils
				.toByteArray(model.getModel()));
		Result result = null;
		ObjectMapper mapper = new ObjectMapper();
		result = mapper.readValue(response, Result.class);
	...
	public String[] getMistakesList() throws Exception {
		Evaluator service = new EvaluatorLocator();
		EvaluatorPortType port = service.getEvaluatorPort();
		String response = port.getActivityDiagramMistakeList();
		String[] model = null;
		ObjectMapper mapper = new ObjectMapper();
	...
	public AD getRefModelDetails(ModelFile model1) throws Exception {
		Evaluator service = new EvaluatorLocator();
		EvaluatorPortType port = service.getEvaluatorPort();
		String response = port.getActivityDiagramDetail(IOUtils
				.toByteArray(model1.getModel()));
		System.out.println(response);
		AD model = null;
		ObjectMapper mapper = new ObjectMapper();
		model = mapper.readValue(response, AD.class);
		return model;
	}
}
