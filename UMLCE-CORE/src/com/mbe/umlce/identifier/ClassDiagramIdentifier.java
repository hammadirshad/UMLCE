...
public class ClassDiagramIdentifier implements Serializable,Identifier{
			...
			if (element.eClass() == UMLPackage.Literals.CLASS) {
				...
				if (!taggedName.contains("NN") && !taggedName.contains("NNP")
					...
					mistakes.add(new Errors(Keywords.Error,
							"Invalid class name", "Class Diagram", clasName,
							"Class name is not noun."));
				}
				...
				if (clas.getRelationships().isEmpty())
					mistakes.add(new Errors(Keywords.Error, "Invalid Class",
							"Class Diagram", clasName,
							"Class having no relationship."));
				// get class attributes
				EList<Property> attribute = clas.getOwnedAttributes();
				if (attribute.isEmpty()) {
					mistakes.add(new Errors(Keywords.Error,
							"Missing Attributes", "Class Diagram", clasName,
							"Class having no attribute."));
				...
				for (Property attr : attribute) {
					if (attr != null) {
						...
						if (attr.getName() == null) {
							mistakes.add(new Errors(Keywords.Error,
									"Null Attribute", "Class Diagram", clasName
											+ "->" + attr.getName(),
									"Attribute name is null."));
						} else {
							// if attribute name is not verb then add it in
							// mistakes
							// list
							String taggedAttr = tagger
									.tagString(attr.getName());
							if (!taggedAttr.contains("NN")
									&& !taggedAttr.contains("NNP")
									&& !taggedAttr.contains("NNS")
									&& !taggedAttr.contains("NNPS"))
								mistakes.add(new Errors(Keywords.Error,
										"Invalid Attribute", "Class Diagram",
										clasName + "->" + attr.getName(),
										"Attribute name is not noun."));
						}

						// checking visibility of attribute
						if (attr.getVisibility().getName()
								.equals(Keywords.Public)) {

							mistakes.add(new Errors(Keywords.Warning,
									"Attribute visibility", "Class Diagram",
									clasName + "->" + attr.getName(),
									"Attribute visibility is public."));
						}
						// checking static members
						if (attr.isStatic()) {

							mistakes.add(new Errors(Keywords.Warning,
									"Static Attribute ", "Class Diagram",
									clasName + "->" + attr.getName(),
						...
						if (attr.getUpperValue() == null
							...
							// PrimitiveTypeImpl pt = (PrimitiveTypeImpl)
							// attr.getType();
							// if(pt==null){
							// mistakes.add("Attribute data type % Class Diagram % "+clasName+"->"+attr.getName()+" % Missing attribute datatype in class.");
							// }
						}
					}
				...
				if (count_1 == count_2) {
					mistakes.add(new Errors(Keywords.Error,
							"Missing Association Name", "Class Diagram",
							clasName, "Class having no association name."));
				}
				// operations
				...
				if (operation.isEmpty()) {
					...
					mistakes.add(new Errors(Keywords.Error,
							"Missing Operations", "Class Diagram", clasName,
							"Class having no opertaion."));
				}
				/*
				 * //check for correct operation name else{ String taggedOp =
				 * null; for(org.eclipse.uml2.uml.Operation op: operation){
				 * String opName = op.getName(); String splitedOp[] ;
				 * if(opName.contains("_")) splitedOp = opName.split("_"); else
				 * if(opName.contains("-")) splitedOp = opName.split("-"); else
				 * if(opName.contains(" ")) splitedOp = opName.split(" ");
				 * taggedOp = tagger.tagString(op.getName());
				 * mistakes.add("Tagged op: "+taggedOp); } }
				 */
				...
				if (element.getName() != null) {
					...
					if (element.getName().isEmpty()) {
						mistakes.add(new Errors("Association name",
								"Static Attribute ", "Class Diagram",
