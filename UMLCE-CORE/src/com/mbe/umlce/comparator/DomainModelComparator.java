...
public class DomainModelComparator implements Serializable,Comparator {
					...
					for (CD s1 : student_1) {
						for (CD s2 : student_2) {
							if (s1.getClassName().equalsIgnoreCase(
									s2.getClassName())) {
								totalPalgiarism += compareAttributes(s1, s2,
										maxCounts);
							}
						}
					}
	 ...
	 */
	private float compareAttributes(CD s1, CD s2, int[] maxCounts) {
		ArrayList<String> attributes_s1 = s1.getAttributes();
		ArrayList<String> attributes_s2 = s2.getAttributes();
		float totalPalgiarism = 0.0f;
		float plagInClass = (float) (50.0 / maxCounts[0]);
		float plagInAttributes = (float) (50.0 / maxCounts[1]);
